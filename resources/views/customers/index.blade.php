...
<div x-data="customerPage()" class="p-6 space-y-6 relative">
    ...
    <!-- HEADER -->
    <div class="bg-gradient-to-r from-gray-800 via-slate-800 to-gray-900 
    ...
    <!-- SEARCH BAR -->
    <div class="flex items-center gap-2 bg-white dark:bg-gray-800 p-3 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition duration-300">
        ...
        <template x-for="(item, index) in filteredCustomers" :key="item.id">
            ...
            >
                <div class="flex items-center gap-3 mb-3">
                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center text-white font-bold shadow-md">
                        <span x-text="item.name.charAt(0).toUpperCase()"></span>
                    </div>
                    <div>
                        <h2 class="text-base font-semibold text-gray-100 group-hover:text-blue-400 transition" x-text="item.name"></h2>
    ...
    <div x-show="selectedCustomer" x-transition.opacity.duration.300ms
        ...
        <div @click.outside="selectedCustomer = null"
            ...
            <div class="flex items-center gap-4 mb-6">
                <div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center text-white font-bold shadow-md">
                ...
                <div>
                    ...
                    <p class="text-gray-400 text-sm" x-text="selectedCustomer.attributes.length + ' nomor terdaftar'"></p>
                </div>
            </div>
                    ...
                    <template x-if="selectedCustomer.attributes.length > 0">
                        <div class="space-y-2">
                            <template x-for="attr in selectedCustomer.attributes" :key="attr.id">
                                <div class="bg-gray-800 border border-gray-700 rounded-lg p-3 hover:border-blue-500 transition">
                                    <p class="text-sm font-semibold text-blue-400" x-text="attr.attribute_value"></p>
                                    <p class="text-xs text-gray-400 italic mt-1" x-text="attr.attribute_notes || '—'"></p>
                                </div>
                            </template>
                        </div>
                    </template>
                    ...
                    <template x-if="selectedCustomer.debts.length > 0">
                        <div class="space-y-2">
                            <template x-for="debt in selectedCustomer.debts" :key="debt.id">
                                <div class="bg-gray-800 border border-gray-700 rounded-lg p-3 hover:border-yellow-500 transition">
                                    <div class="flex justify-between text-sm">
                                        <p class="font-semibold text-yellow-400">Nota: <span x-text="debt.nomor_nota"></span></p>
                                        <p class="text-gray-400 text-xs" x-text="formatLongDate(debt.created_at)"></p>
                                    </div>
                                    <p class="text-sm text-gray-300 mt-1">
                                        Produk: <span class="font-semibold text-gray-100" x-text="debt.details?.[0]?.product?.name || '—'"></span>
                                    </p>
                                    <p class="text-sm font-bold text-yellow-400 mt-1">Rp <span x-text="Number(debt.subtotal).toLocaleString('id-ID')"></span></p>

                                    <!-- Tombol Lunaskan -->
                                    <div class="flex justify-end mt-3">
                                        <button
                                            @click.stop="openPayConfirm(debt, selectedCustomer)"
...
function customerPage() {
    return {
        ...
        openCustomer(cust) { this.selectedCustomer = cust; },
