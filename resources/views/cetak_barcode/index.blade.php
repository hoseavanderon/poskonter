...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10"
     x-data="barcodePage()" x-init="init()">
    ...
    <div class="flex items-center gap-3 mb-8">
        <div class="bg-slate-800 rounded-lg p-3 shadow-sm border border-slate-700 flex items-center justify-center">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none"
                 xmlns="http://www.w3.org/2000/svg" class="relative top-[1px]">
                ...
                <tbody class="divide-y divide-slate-700">
                        ...
                        <tr class="hover:bg-slate-700/40 transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-md bg-slate-700 flex items-center justify-center text-sm text-slate-300 font-medium">
                                        <span x-text="getInitials(item.name)"></span>
                                    </div>
                                    <div>
                                        <div class="text-sm font-medium text-slate-100">
                                            <span x-text="item.name"></span>
                                            <span x-show="item.variant" class="text-slate-400 text-xs ml-1">(<span x-text="item.variant"></span>)</span>
                                        </div>
                                        <div class="text-xs text-slate-500" x-text="'Barcode: ' + (item.barcode ?? '-')"></div>
                                    </div>
                                </div>
                            </td>

                            <td class="px-4 py-3 text-sm text-slate-300" x-text="item.brand + ' • ' + item.category"></td>
                            <td class="px-4 py-3 text-right text-sm text-slate-400" x-text="item.stock"></td>

                            <td class="px-4 py-3 text-center">
                                <input type="number" min="1"
                                    x-model.number="item.qty"
                                    @blur="updateQty(item, item.qty)"
                                    class="qty-input"
                                    placeholder="1">
                            </td>
                    ...
                    <tr x-show="!cart.length">
                        <td colspan="5" class="px-6 py-8 text-center text-slate-500">
                            Keranjang cetak kosong — klik <span class="font-medium text-slate-100">Pilih Produk</span> untuk menambah.
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <div x-show="modalOpen"
        ...
        <div @click.outside="closeModal()" class="bg-slate-800 w-full max-w-4xl rounded-xl border border-slate-700 shadow-lg overflow-hidden">
            ...
            <div class="p-4 border-b border-slate-700">
                <input type="text" x-model="modalSearch" placeholder="Cari produk berdasarkan nama, merek, atau kategori..."
            ...
            <div class="max-h-96 overflow-y-auto">
                <template x-for="p in availableProducts" :key="p.id">
                    <div @click="selectProduct(p)"
                        ...
                        <div class="flex items-center gap-3">
                            <div class="w-9 h-9 bg-slate-700 rounded-md flex items-center justify-center text-xs text-slate-200 font-medium">
                                <span x-text="getInitials(p.name)"></span>
                            </div>
                            <div>
                                <div class="text-sm text-slate-100 font-medium">
                                    <span x-text="p.name"></span>
                                    <span x-show="p.variant" class="text-slate-400 text-xs ml-1">(<span x-text="p.variant"></span>)</span>
                                </div>
            ...
            <div class="flex items-center justify-end p-4 border-t border-slate-700 bg-slate-800">
                <button @click="closeModal()" class="px-3 py-2 bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-md text-sm">
                    Tutup
                </button>
            </div>
        </div>
    ...
    <div 
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 text-yellow-400 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-.01-9a9 9 0 110 18 9 9 0 010-18z" />
        </svg>
...
function barcodePage() {
    return {
        ...
        openModal() { this.modalSearch = ''; this.modalOpen = true; },
        closeModal() { this.modalOpen = false; },
        selectProduct(p) { this.cart.push({ ...p, qty: 1 }); },
        ...
        printCart() {
            ...
            for (const item of this.cart) {
                ...
                if (qty > stock) {
                    this.showPopup(`Jumlah cetak melebihi stok tersedia untuk "${item.name}". (Stok: ${stock}, Cetak: ${qty})`);
                    return;
                }
            }
